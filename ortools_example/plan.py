...
class Planner:
    ...
    def build_model(self) -> None:
        ...
        for product_name in self.product_names:
            ...
            sum_inputs = sum(
                ...
                * self.product_map[other_product_name].ingredients[product_name].amount
        ...
        sum_labor = sum(
            ...
            * self.product_map[product_name].required_labor
            for product_name in self.product_names
        )
        ...
        sum_envimpact = sum(
            self.gross_production_of[product_name]
            * self.product_map[product_name].envimpact
            for product_name in self.product_names
        )
        ...
        for product_name in self.product_names:
            self.solver.Add(
                self.net_production_of[product_name]
                >= self.product_map[product_name].minimum,
    ...
    def output_solution(self, debug: bool) -> None:
        ...
        print(
            f"Total labor: {self.total_labor.solution_value():0.3f}{self.labour_resource.unit}"
        )
        print(
